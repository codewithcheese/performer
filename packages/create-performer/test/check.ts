...
try {
  ...
  const vite_version = JSON.parse(ls_vite_result.toString())[0].devDependencies
    .vite.version;
...
fs.writeFileSync(
  path.join(test_workspace_dir, "pnpm-workspace.yaml"),
  "packages:\n  - ./*\n",
);
...
const templates =
  /** @type {Array<'default' | 'skeleton' | 'skeletonlib'>} */ fs.readdirSync(
    "templates",
  );
...
for (const template of templates) {
  ...
  for (const types of ["typescript" as const]) {
    const cwd = path.join(test_workspace_dir, `${template}-${types}`);
    fs.rmSync(cwd, { recursive: true, force: true });

    create(cwd, {
      name: `create-performer-test-${template}-${types}`,
      template: "default",
      types,
    });
