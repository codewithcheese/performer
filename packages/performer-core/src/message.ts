...
export const fromLangchain = {
	transform(chunk: BaseMessage, controller: TransformStreamDefaultController<CoreMessage>) {
...
export function toLangchain(messages: CoreMessage[]) {
...
export function isImageContent(content: unknown): content is MessageImageUrlContent {
	return (
		!!content &&
		typeof content === 'object' &&
		'type' in content &&
...
export function isMessage(message: unknown): message is CoreMessage {
	return (
		message != null && typeof message === 'object' && 'role' in message && 'content' in message
	);
}
...
export function readTextContent(message: CoreMessage) {
