import { type CoreNode, getNearestParent } from '../node.js';
...
export function createContextId<STATE = unknown>(name: string): ContextId<STATE> {
	return Object.freeze({
		name
	} as any);
}

export function initContext<STATE extends unknown>(context: ContextId<STATE>, initValue: STATE) {
	const contextKey = `context-${context.name}` as const;
	const { value } = useHook<Signal<STATE>>(contextKey, new Signal(initValue));
	return value;
}

export function useContext<STATE extends unknown>(context: ContextId<STATE>): Signal<STATE> {
	const providerKey = `provider-${context.name}` as const;
	const contextKey = `context-${context.name}` as const;
	const { value, set, scope } = useHook<CoreNode | undefined>(providerKey, undefined);
	let provider = value;
	if (provider == null) {
		provider = getNearestParent(scope.node, (parent) => contextKey in parent.hooks);
	}
	if (provider == null) {
...
export type ProviderHookRecord = {
	[key in ProviderHookKey]?: CoreNode;
};
