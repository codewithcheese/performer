...
export class RunSession {
	...
	node?: CoreNode;
	errors: Error[] = [];
	inputQueue: CoreMessage[] = [];
	inputNode: CoreNode | undefined;
	...
	nodes: Map<CoreElement, CoreNode> = new Map();
	...
	constructor({ id, element, node, throwOnError }: RunProps) {
		...
		this.waitForInput = new Promise<void>((resolve) => (this.inputResolver = resolve))
			.catch(console.error)
			.finally(() => console.log('wait for input complete'));
		this.throwOnError =
			throwOnError === undefined ? process.env['VITEST'] !== undefined : throwOnError;
	}
	...
	async waitUntilSettled() {
		return Promise.race([this.waitForInput, this.waitUntilFinished]).finally(() => {
			this.waitForInput = new Promise<void>((resolve) => (this.inputResolver = resolve)).catch(
				console.error
			);
		});
	}
