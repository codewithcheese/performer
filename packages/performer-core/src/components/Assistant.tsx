...
import { CoreEvent, createMessageEvent } from '../event.js';
...
export const Assistant: Component<AssistantProps> = async ({
	...
	if (content) {
		...
		const transformStream = new TransformStream<BaseMessage, CoreMessage>(fromLangchain);
		const message = await handleChatModelResponse(iterable.pipeThrough(transformStream), announce);
		if (message) {
			newMessages.push(message);
			onMessage(message);
		}
		...
		if (isAssistantMessage(message) && message.tool_calls) {
			for (const toolCall of message.tool_calls) {
				...
				const toolMessage = await tool.call(JSON.parse(toolCall.function.arguments));
				if (toolMessage) {
...
function isAsyncIterable(obj: unknown): obj is AsyncIterable<unknown> {
	return !!obj && typeof (obj as AsyncIterable<unknown>)[Symbol.asyncIterator] === 'function';
}

async function handleChatModelResponse(
	msgOrStream: ReadableStream<unknown>,
	announce: (event: CoreEvent) => void
): Promise<CoreMessage | undefined> {
	let aggregate: CoreMessage | undefined;
				...
				if (!aggregate) {
					...
					if (
						...
						for (const [index, toolCall] of chunk.tool_calls.entries()) {
							aggregate.tool_calls[index].function.arguments += toolCall.function.arguments;
						}
					}
					...
					if (isAssistantMessage(chunk)) {
						...
						for (const [i, chunkEntry] of chunk.content.entries()) {
							...
							} else if (isImageContent(chunkEntry) && isImageContent(aggregateEntry)) {
