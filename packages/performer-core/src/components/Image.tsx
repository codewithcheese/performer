...
export const Image: Component<Props> = async (
  { content, model = "dall-e-3", numberOfImages = 1, instructions },
  use,
) => {
  ...
  const toolCall = await use(async (controller) => {
    ...
    const response = await chat.invoke(lcMessages, {
      tool_choice: {
        type: "function",
        function: { name: imagePromptFunctionName },
      },
    });
    return response.additional_kwargs.tool_calls?.find(
