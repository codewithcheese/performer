...
type IntrinsicProps = { onMessage?: (message: CoreMessage) => void };
...
export namespace JSX {
	...
	export type IntrinsicElements = {
		user: IntrinsicProps & ({ content: UserMessage['content'] } | { children: string });
		...
		tool: IntrinsicProps &
			({ id: ToolMessage['id']; content?: ToolMessage['content'] } | { children: string });
	};
}
...
export function jsx<Type extends Component<P>, P extends Props>(type: Type, props: P): CoreElement {
	if (type === undefined) {
		// @ts-ignore
		type = Fragment;
	}
	if (props.children == null) {
		...
		if (Array.isArray(props.children)) {
			const countString = props.children.filter((prop: any) => typeof prop === 'string').length;
			if (countString && countString !== props.children.length) {
				// throw if only subset is string
				throw Error(`Element children cannot contain both text and objects. Found in ${type.name}`);
			} else if (countString) {
