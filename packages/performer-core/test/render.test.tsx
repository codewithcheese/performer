...
import { CoreMessage, resolveMessages, RunSession, useState } from '../src/index.js';
...
test('should update prop when signal changes', async () => {
	...
	function App() {
		const message = useState<CoreMessage | null>(null);
		return () => (
			<>
				<user
...
test('should update and run message actions when state changes', async () => {
	...
	session.start();
	await session.waitUntilSettled();
	let messages = resolveMessages(session.node);
	expect(messages).toHaveLength(1);

	resetFinished(session);
	session.node!.child!.nextSibling!.hooks['state-0'].value = true;

	await session.waitUntilSettled();
	messages = resolveMessages(session.node);
	expect(messages).toHaveLength(3);
});
...
test('should update links when elements are reordered', async () => {
	...
	let messages = resolveMessages(session.node, undefined, { showResolveMessages: true });
	expect(messages).toHaveLength(3);

	resetFinished(session);
	const offset = session.node!.hooks['state-0'];
	offset!.value += 1;

	await session.waitUntilSettled();
	messages = resolveMessages(session.node, undefined, { showResolveMessages: true });
	expect(messages).toHaveLength(3);
});
...
test('should render new elements when dynamically added or removed', async () => {
	...
	resetFinished(session);
	const times = session.node!.hooks['state-0'];
	times.value += 4;
...
test('should unlink messages when removed by conditional', async () => {
	...
	await session.waitUntilSettled();
	let messages = resolveMessages(session.node, undefined, { showResolveMessages: true });
	expect(messages.length, 'Expect 2 messages before they are unlinked by `If`').toEqual(2);

	resetFinished(session);
	const predicate = session.node!.hooks['state-0'];
	predicate.value = false;

	await session.waitUntilSettled();
	messages = resolveMessages(session.node, undefined, { showResolveMessages: true });
	expect(messages.length, 'Expect 0 messages after they are unlinked by `If`').toEqual(0);
});
...
test('should render tree', async () => {
	...
	expect(root.child?.nextSibling?.child?.nextSibling?.type.name).toEqual('Third');
	assert(root.child?.nextSibling?.child?.nextSibling?.child?.type instanceof Function);
	expect(root.child?.nextSibling?.child?.nextSibling?.child?.type.name).toEqual('Child');
