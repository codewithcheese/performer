...
test("should support wildcard", () => {
  ...
  eventTarget.dispatchEvent(new PerformerErrorEvent("root", { message: "1" }));
  expect(count).toEqual(1);
  expect(errorCount).toEqual(1);
  eventTarget.dispatchEvent(new PerformerErrorEvent("root", { message: "2" }));
  expect(count).toEqual(2);
  expect(errorCount).toEqual(2);
  eventTarget.removeEventListener("*", listener);
  eventTarget.dispatchEvent(new PerformerErrorEvent("root", { message: "3" }));
  expect(count).toEqual(2);
